...
/// What kind of notification this is. M2 ships only `NewAwaiting`.
...
pub(crate) fn redact_send_error(e: anyhow::Error) -> anyhow::Error {
    anyhow::anyhow!("telegram send failed: {e}")
}
...
mod tests {
    ...
    fn test_send_error_redacts_bot_token() {
        ...
        let leaky = anyhow::anyhow!(
            "error sending request for url (https://api.telegram.org/bot{sentinel}/sendMessage)"
        );
